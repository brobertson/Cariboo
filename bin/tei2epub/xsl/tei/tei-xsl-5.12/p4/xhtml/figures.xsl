...
<xsl:stylesheet xmlns="http://www.w3.org/1999/xhtml"
  ...
  <xd:doc type="stylesheet">
    ...
    <xd:cvsId>$Id: figures.xsl 4572 2008-05-17 15:20:00Z rahtz $</xd:cvsId>
  ...
  <xsl:template match="figure">
	...
	<xsl:call-template name="showGraphic"/>
      ...
      <xsl:when test="parent::ref or parent::xref">
	<xsl:call-template name="showGraphic"/>
      </xsl:when>
      <xsl:otherwise>
	<div>
	  ...
	  <xsl:if test="head">
	    <div class="caption">
	      <xsl:choose>
		<xsl:when
		    test="ancestor::front and  $numberFrontFigures='true'">
		  <xsl:call-template name="i18n">
		    <xsl:with-param name="word">figureWord</xsl:with-param>
		  </xsl:call-template>
		  <xsl:text> </xsl:text>
		  <xsl:number count="figure[head]" from="front"
			      level="any"/>
		  <xsl:text>. </xsl:text>
		</xsl:when>
		<xsl:when test="ancestor::back and $numberBackFigures='true'">
		  <xsl:call-template name="i18n">
		    <xsl:with-param name="word">figureWord</xsl:with-param>
		  </xsl:call-template>
		  <xsl:text> </xsl:text>
		  <xsl:number count="figure[head]" from="back" level="any"/>
		  <xsl:text>. </xsl:text>
		</xsl:when>
		<xsl:when test="ancestor::body and $numberFigures='true'">
		  ...
		  <xsl:text> </xsl:text>
		  <xsl:number count="figure[head]" from="body" level="any"/>
		  <xsl:text>. </xsl:text>
		</xsl:when>
	      </xsl:choose>
  ...
  <xd:doc>
    <xd:short>Process elements figure/head</xd:short>
    <xd:detail> </xd:detail>
  </xd:doc>
  ...
  <xsl:template match="graphic">
    <xsl:if test="@id and not($xhtml='true')">
	<a class="g" name="{@id}">
	  <xsl:comment> </xsl:comment>
	</a>
    </xsl:if>
  ...
  <xd:doc>
    <xd:short>Process elements table[@rend='simple']</xd:short>
    <xd:detail> </xd:detail>
  </xd:doc>
  ...
  <xsl:template name="showGraphic">
    ...
    <xsl:variable name="Alt">
      <xsl:choose>
        ...
        <xsl:when test="parent::figure/figDesc">
          <xsl:for-each select="parent::figure/figDesc">
	    <xsl:apply-templates mode="plain"/>
	  </xsl:for-each>
        </xsl:when>
    ...
    <xsl:choose>
	  ...
	  <xsl:text> </xsl:text>
	  <xsl:for-each select="self::figure|parent::figure">
	    <xsl:number count="figure[head]" level="any"/>
	  </xsl:for-each>
	  <xsl:text> </xsl:text>
	  <xsl:value-of select="$File"/>
	  <xsl:text> [</xsl:text>
