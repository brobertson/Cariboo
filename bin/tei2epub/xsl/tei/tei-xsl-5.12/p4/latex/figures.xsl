...
<xsl:stylesheet exclude-result-prefixes="xd exsl estr edate a rng tei teix"
  ...
  <xd:doc type="stylesheet">
    ...
    <xd:cvsId>$Id: figures.xsl 4696 2008-07-03 10:38:25Z rahtz $</xd:cvsId>
  ...
  <xd:doc>
    <xd:short>Process elements table[@rend='display']</xd:short>
    <xd:detail> </xd:detail>
  </xd:doc>
  <xsl:template match="table[@rend='display']" mode="xref">
    <xsl:text>Table </xsl:text>
    <xsl:number count="table[@rend='display']" level="any"/>
  </xsl:template>
  <xd:doc>
    <xd:short>Process elements table[@rend='display']</xd:short>
    <xd:detail> </xd:detail>
  </xd:doc>
  <xsl:template match="table[@rend='display']">
	...
	<xsl:text>}{}</xsl:text>
     ...
     <xsl:text>\end{tabular} 
      \caption{</xsl:text>
      <xsl:apply-templates mode="ok" select="head"/>
      <xsl:text>}
     \end{small} 
     \end{center}
     \end{table}</xsl:text>
  ...
  <xsl:template name="makeTable">
    ...
    <xsl:choose>
      <xsl:when test="head and not(@rend='display')">
...
<xsl:template name="tableHline">
    <xsl:choose>
      <xsl:when test="ancestor::table or @rend='display'"> \hline </xsl:when>
