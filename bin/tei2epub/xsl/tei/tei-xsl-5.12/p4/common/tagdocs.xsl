...
<xsl:stylesheet xmlns="http://www.tei-c.org/ns/1.0"
  ...
  <xd:doc type="stylesheet">
    ...
    <xd:cvsId>$Id: tagdocs.xsl 4684 2008-06-30 12:42:34Z rahtz $</xd:cvsId>
 ...
 <xsl:key name="CHILDMOD" match="Element" use="@module"/>
  ...
  <xsl:template match="attDef/datatype" mode="weave">
    <xsl:element namespace="{$outputNS}" name="{$rowName}">
      <xsl:element namespace="{$outputNS}" name="{$cellName}">
        <xsl:attribute name="{$rendName}">
          <xsl:text>odd_label</xsl:text>
      ...
      <xsl:element namespace="{$outputNS}" name="{$tableName}">
		...
		<xsl:with-param name="name">
		  <xsl:value-of select="$name"/>
              ...
              <xsl:call-template name="makeDescription"/>
              <xsl:if test="listRef">
                <xsl:for-each select="listRef/ptr">
                  <xsl:text> </xsl:text>
  ...
  <xsl:template match="classes" mode="weave"> </xsl:template>
  ...
  <xsl:template match="elementSpec" mode="weavebody">
	...
	<xsl:with-param name="name">
	  <xsl:text>&lt;</xsl:text>
	  <xsl:choose>
	    <xsl:when test="content/rng:empty">
  ...
  <xsl:template match="exemplum" mode="doc">
...
<!--	<xsl:message>reject exemplum <xsl:value-of select="."/> cos 1</xsl:message>-->
      ...
      <xsl:when test="not(@xml:lang) and
		      ...
		      and
		      ../exemplum[@xml:lang='$documentationLanguage']">
<!--      	<xsl:message>reject exemplum <xsl:value-of select="."/> cos 2</xsl:message>-->
      </xsl:when>
      <xsl:when test="parent::attDef">
	<xsl:element namespace="{$outputNS}" name="{$rowName}">
	  <xsl:element namespace="{$outputNS}" name="{$cellName}">
  ...
  <xsl:template match="item">
    <xsl:choose>
      <xsl:when test="parent::list[@type='gloss'] or preceding-sibling::label">
        <xsl:element namespace="{$outputNS}" name="{$ddName}">
          <xsl:apply-templates/>
        </xsl:element>
      </xsl:when>
  ...
  <xd:doc>
    <xd:short>Process remarks</xd:short>
    <xd:detail> </xd:detail>
  </xd:doc>
  ...
  <xsl:template match="valDesc" mode="weave">
    <xsl:element namespace="{$outputNS}" name="{$rowName}">
      <xsl:element namespace="{$outputNS}" name="{$cellName}">
        <xsl:attribute name="{$rendName}">
          <xsl:text>odd_label</xsl:text>
  ...
  <xd:doc>
    <xd:short>Process elements val</xd:short>
    <xd:detail> </xd:detail>
  </xd:doc>
  ...
  <xd:doc>
    <xd:short>Process elements tag</xd:short>
    <xd:detail> </xd:detail>
  </xd:doc>
        ...
        <xsl:choose>
          <xsl:when test="not($localsource='')">
            <xsl:for-each select="document($localsource)/TEI.2">
              <xsl:for-each select="elementSpec[classes/memberOf[@key=$this]]">
  ...
  <xsl:template name="generateParentsByElement">
    <xsl:variable name="this" select="@ident"/>
    <xsl:for-each select="key('REFS',$this)/ancestor::elementSpec">
      <xsl:sort select="@ident"/>
      <xsl:call-template name="linkTogether">
        <xsl:with-param name="name" select="@ident"/>
  ...
  <xsl:template name="generateParentsByAttribute">
	...
	<xsl:for-each select="key('ATTREFS-CLASS',$this)">
	  ...
	  <xsl:sort select="@ident"/>
	  <xsl:element namespace="{$outputNS}" name="{$itemName}">
	    <xsl:for-each select="ancestor::classSpec">
	      <xsl:call-template name="linkTogether">
		<xsl:with-param name="name">
		  <xsl:value-of select="@ident"/>
		</xsl:with-param>
	...
	<xsl:for-each select="key('ATTREFS-ELEMENT',$this)">
	  ...
	  <xsl:sort select="@ident"/>
	  <xsl:element namespace="{$outputNS}" name="{$itemName}">
	    <xsl:for-each select="ancestor::elementSpec">
	      <xsl:call-template name="linkTogether">
		<xsl:with-param name="name">
		  <xsl:value-of select="@ident"/>
		</xsl:with-param>
  ...
  <xsl:template name="generateParentsByMacro">
    ...
    <xsl:if test="key('MACROREFS',$this)">
      <xsl:for-each select="key('MACROREFS',$this)/ancestor::macroSpec">
        <xsl:sort select="@ident"/>
	<xsl:call-template name="showSpace"/>
        <xsl:call-template name="linkTogether">
          <xsl:with-param name="name" select="@ident"/>
    ...
    <!--
      <xsl:for-each select="key('REFS',@ident)/ancestor::elementSpec">
	<xsl:call-template name="linkTogether">
	  <xsl:with-param name="name" select="@ident"/>
	</xsl:call-template>
      </xsl:for-each>
...
<xsl:template match="classSpec|elementSpec|macroSpec" mode="weave">
  <xsl:call-template name="refdoc"/>
</xsl:template>


<xsl:template match="divGen[@type='modelclasscat']">
    <xsl:apply-templates mode="weave" select="key('MODELCLASSDOCS',1)">
      <xsl:sort select="@ident"/>
    </xsl:apply-templates>
  </xsl:template>

<xsl:template match="divGen[@type='attclasscat']">
  ...
  <xsl:template match="divGen[@type='elementcat']">
    <xsl:apply-templates mode="weave" select="key('ELEMENTDOCS',1)">
      <xsl:sort select="@ident"/>
    </xsl:apply-templates>
  </xsl:template>

  <xsl:template match="divGen[@type='attcat']">
    <xsl:element namespace="{$outputNS}" name="{$tableName}">
	...
	<xsl:if test="generate-id()=generate-id(key('ATTRIBUTES',$this)[1])">
	  <xsl:element namespace="{$outputNS}" name="{$rowName}">
	    ...
	    <xsl:for-each select="key('ATTRIBUTES-CLASS',$this)">
	      <xsl:sort select="ancestor::classSpec/@ident"/>
	      <xsl:for-each select="ancestor::classSpec|ancestor::elementSpec">
		<xsl:call-template name="linkTogether">
		  <xsl:with-param name="name">
		    <xsl:value-of select="@ident"/>
		  </xsl:with-param>
  ...
  <xd:doc>
    <xd:short>No-op processing of elements gloss and desc in
    normal modes, as they will always be called explicitly if
    needed.</xd:short>
    <xd:detail> </xd:detail>
  </xd:doc>

  <xsl:template match="desc|gloss" mode="weave"/>
  ...
  <xsl:template match="remarks" mode="weave">
    ...
    <xsl:choose>
      <xsl:when test="preceding-sibling::remarks"/>
      <xsl:when test="count(../remarks)=1">
	<xsl:apply-templates select="." mode="doc"/>
      </xsl:when>
  ...
  <xsl:template match="valList" mode="weave">
  ...
  <xsl:template match="defaultVal" mode="weave" />
...
<xsl:template name="showAttClasses">
  ...
  <xsl:variable name="clatts">
    <xsl:for-each
	select="ancestor-or-self::elementSpec|ancestor-or-self::classSpec">
  ...
  <xsl:choose>
	...
	<xsl:if test="ancestor::schemaSpec and key('CLASSES','att.global')">
	...
	<xsl:copy-of select="$clatts"/>
      ...
      <xsl:when test="ancestor::schemaSpec and
	  ...
	  <xsl:when test="key('CLASSES',@key)">
		...
		<xsl:if test=".//attDef">
		  ...
		  <xsl:for-each select=".//attDef">
		    ...
		    <xsl:if test="following-sibling::attDef">
		      <xsl:text>, </xsl:text>
		    </xsl:if>
		  </xsl:for-each>
		  <xsl:text>)</xsl:text>
  ...
  <xsl:template name="processSpecDesc">
    ...
    <xsl:choose>
      ...
      <xsl:when test="not($localsource='')">
        <xsl:for-each select="document($localsource)/TEI.2">
          <xsl:apply-templates mode="show" select="*[@ident=$name]">
            <xsl:with-param name="atts" select="$atts"/>
          </xsl:apply-templates>
        </xsl:for-each>
      </xsl:when>
      <xsl:otherwise>
        ...
        <xsl:apply-templates mode="show" select="document($loc)/*">
          <xsl:with-param name="atts" select="$atts"/>
        </xsl:apply-templates>
      </xsl:otherwise>
    </xsl:choose>
  ...
  <xsl:template name="makeTagsetInfo">
    ...
    <xsl:for-each
	select="key('MODULES',@module)/ancestor::div[last()]">
      <xsl:text> — </xsl:text>
    ...
    <xsl:if test="$verbose='true'">
      <xsl:message> tagset <xsl:value-of select="@id"/>: <xsl:value-of
  ...
  <xsl:template name="generateChildren">
	...
	<xsl:call-template name="i18n">
      ...
      <xsl:when test="content/rng:text and count(content/rng:*)=1">
	<xsl:call-template name="i18n">
