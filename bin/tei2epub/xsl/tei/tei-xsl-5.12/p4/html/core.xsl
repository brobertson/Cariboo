...
<xsl:stylesheet
  ...
  <xsl:template match="gap"> [...]<xsl:apply-templates/>
  </xsl:template>
  <xd:doc>
  ...
  <xd:doc>
    <xd:short>Process element title</xd:short>
    <xd:detail> </xd:detail>
  </xd:doc>
  <xd:doc>
      ...
      <xsl:otherwise>
		...
		level="any"/>
          ...
          <xsl:otherwise>
            <xsl:choose>
              <xsl:when test="ancestor::front">
                <xsl:number count="note[./@place='foot']" from="front"
                  level="any"/>
              </xsl:when>
              <xsl:when test="ancestor::back">
                <xsl:number count="note[./@place='foot']" from="back"
                  level="any"/>
              </xsl:when>
              <xsl:otherwise>
                <xsl:number count="note[./@place='foot']" from="body"
                  level="any"/>
              </xsl:otherwise>
            </xsl:choose>
          </xsl:otherwise>
  ...
  <xsl:template name="printNotes">
      <xsl:choose>
	<xsl:when test="$footnoteFile='true' and ancestor-or-self::TEI.2/text/descendant::note[@place='foot' or @place='end']">
">
	...
	<xsl:when test="self::div">
	  ...
	  <xsl:if test="child::*[not(self::div)]/descendant::note[@place='foot' or @place='end']">
	    <div class="notes">
		...
		<xsl:call-template name="i18n">
	      ...
	      <xsl:apply-templates 
		  mode="printnotes"
		  select="child::*[not(self::div)]/descendant::note[@place='foot' or @place='end']"/>
	    </div>
	  </xsl:if>
	    </xsl:when>
	    <xsl:otherwise>
	  <xsl:if test="descendant::note[@place='foot' or @place='end']">
	    <div class="notes">
	      <div class="noteHeading">
		<xsl:call-template name="i18n">
		  <xsl:with-param name="word">noteHeading</xsl:with-param>
	      ...
	      <xsl:apply-templates 
		  mode="printnotes"
		  select="descendant::note[@place='foot' or @place='end']"/>
	    </div>
	  </xsl:if>
	    </xsl:otherwise>
	  </xsl:choose>
	...
	<xsl:when test="$splitLevel &gt;-1 and self::text"/>
	<!--
	    ...
	    <xsl:for-each
	    select="descendant::note[(@place='foot' or @place='end') and
	    not(ancestor::div)]">
    ...
    <xsl:if
	test="ancestor-or-self::TEI.2/text/descendant::app">
  ...
  <xsl:template name="makeAnchor">
	...
	<a name="{$name}">
      ...
      <xsl:when test="@id and $xhtml='true'">
	<span>
	  <xsl:attribute name="id">
	    <xsl:value-of select="@id"/>
	  </xsl:attribute>
