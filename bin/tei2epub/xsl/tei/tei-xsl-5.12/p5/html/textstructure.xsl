...
-->
  ...
  <xsl:template match="tei:div[@type='canto']">
    ...
    <xsl:element name="h{$divlevel + $divOffset}"><xsl:attribute name="hieverybody">557</xsl:attribute>
  ...
  <xsl:template name="doBody">
    ...
    <xsl:choose>
      <xsl:when test="parent::tei:div/@rend='multicol'">
        <td valign="top">
          <xsl:if test="not($Head = '')">
            <xsl:element name="h{$Head + $divOffset}"><xsl:attribute name="hieverybody">938</xsl:attribute>
      ...
      <xsl:otherwise>
        <xsl:if test="not($Head = '')">
          <xsl:element name="h{$Head + $divOffset}"><xsl:attribute name="hieverybody">960</xsl:attribute>
            <xsl:if test="$xhtml='false'">
              <a name="{$ident}"/>
            </xsl:if>
            <xsl:call-template name="header">
  ...
  <xsl:template name="doDivBody">
    ...
    <xsl:choose>
      <xsl:when test="parent::tei:div/@rend='multicol'">
        <td valign="top">
          <xsl:if test="not($Type = '')">
            <xsl:element name="h{$Type + $divOffset}"><xsl:attribute name="hieverybody">989</xsl:attribute>
              <xsl:if test="$xhtml='false'">
                <a name="{$ident}"/>
              </xsl:if>
              <xsl:call-template name="header">
      ...
      <xsl:otherwise>
        <xsl:if test="not($Type = '')">
          <xsl:element name="h{$Type + $divOffset}"><xsl:attribute name="hieverybody">1012</xsl:attribute>
            <xsl:if test="$xhtml='false'">
              <a name="{$ident}"/>
            </xsl:if>
            <xsl:call-template name="header">
